...
- **2026-02-16:** Wrote `internal/graders/diff_grader_test.go` with 48 test cases (11 top-level + 37 subtests) for the diff grader (#99). Tests cover: exact snapshot match/mismatch, diff fragment checking (+added/-removed lines), missing fragments with partial scores, file not found, empty WorkspaceDir graceful error, multiple files with partial scoring, whitespace handling (trailing spaces, trailing newlines, double spaces), edge cases (empty files, binary content, large files, unprefixed fragments), path traversal rejection, result details and duration, combined content+fragment checks. Written ahead of Linus's implementation — tests reference `NewDiffGrader`, `DiffGraderArgs`, `ExpectedFile`, `diffGrader`, and `models.GraderKindDiff` which don't exist yet. File is gofmt-clean.
- Diff grader API assumption: `ExpectedFile` struct has `Path` (workspace-relative), `Content` (exact expected string for snapshot), and `ContainsDiff` (slice of `+line`/`-line` fragments). `+` prefix = must be present, `-` prefix = must be absent, no prefix = must be present.
- Diff grader follows same patterns as file_grader: `validatePathInWorkspace` for traversal, `measureTime` for duration, zero-score with nil error for empty WorkspaceDir, partial scoring via passedChecks/totalChecks.

📌 Team update (2026-02-17): Diff grader needs unit tests (diff_grader_test.go): snapshot match/mismatch, diff fragments, missing file, path traversal. — flagged by Rusty
📌 Team update (2026-02-17): Workspace resource setup (setupWorkspaceResources) needs security tests: normal write, absolute path rejection, traversal rejection. — flagged by Rusty

- **2026-02-18:** Wrote comprehensive tests for baseline skill impact feature (#194). Created `internal/models/baseline_test.go` (6 tests: YAML serialization for Baseline field, SkillImpactMetric JSON serialization, omitempty behavior) and `internal/orchestration/baseline_test.go` (14 tests: computePassRate with 7 cases, computeSkillImpact with 7 cases including division guard, mergeBaselineOutcomes with 3 scenarios including task mismatch errors, 3 skipped integration tests for full baseline runs awaiting task loading setup). All tests follow table-driven pattern from existing runner tests. Tests use actual Runs slice (not Passed/Trials fields) per current TestOutcome structure. Models tests pass (6/6). Orchestration tests will compile once Linus fixes linter issues (redundant newlines in fmt.Println). Implementation already exists in runner.go (lines 275-344) — tests written proactively ahead of merge.
- Key test coverage: division-by-zero guard (0.01 floor per design spec), negative delta (skills hurt), both-fail zero delta, perfect scores, single trial, task set mismatches, JSON omitempty tags, YAML bool deserialization.
