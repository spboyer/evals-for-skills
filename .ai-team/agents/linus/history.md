...
### #459 — Fix skill_invocation grader 0 skills detected (PR #465)
- **Date:** 2026-02-27
- **Branch:** `squad/459-fix-skill-invocation`
- **Files changed:** `internal/execution/copilot.go`, `internal/execution/skill_injection_test.go` (new)
- **What:** The copilot-sdk executor passed `SkillDirectories` to the SDK's `SessionConfig` but never injected skill definitions into the agent's system message. The agent had no visibility into available skills, so it never invoked them, and the `skill_invocation` grader always reported 0. Fix: read SKILL.md files from skill directories, extract name+description from frontmatter, and append `<available_skills>` to the system message via `SystemMessageConfig` (mode: append). Applies to both `CreateSession` and `ResumeSessionWithOptions`. 19 new tests.
- **Key learning:** `SkillDirectories` in the SDK handles skill routing (so the SDK knows where SKILL.md files live), but the agent model also needs explicit skill definitions in its system message context to know *what skills are available* and *when to invoke them*. The SDK's `SystemMessageConfig` with `mode: "append"` is the correct way to inject additional context without replacing the SDK's built-in system prompt. A lightweight frontmatter parser (line-by-line string extraction) avoids pulling in the `skill` package dependency from the execution layer.
