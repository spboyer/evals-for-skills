...
### #460/#461 — Discovery evals and symlink following (PR pending)
- **Date:** 2026-02-26
- **Branch:** `squad/460-fix-discovery`
- **Files changed:** `internal/discovery/discovery.go`, `internal/discovery/discovery_test.go`, `README.md`, `site/src/content/docs/reference/cli.mdx`
- **What:** Added evals/eval.yaml discovery, resolved root symlinks, switched to filepath.Walk for symlink traversal, added evals coverage, and updated docs.
- **Key learning:** `filepath.Walk` follows symlinks while `WalkDir` does not; resolve root symlinks with `filepath.EvalSymlinks` to keep discovery working when skills live under junctions.
