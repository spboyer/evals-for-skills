...
### Docs Audit Fixes (PR #455)
- **Date:** 2026-02-27
- **Branch:** `squad/docs-graders-eval-yaml`
- **Files changed:** `site/src/content/docs/guides/graders.mdx`, `site/src/content/docs/guides/eval-yaml.mdx`, `site/src/content/docs/reference/releases.mdx`
- **What:** Fixed three documentation gaps: added `tool_constraint` grader docs (updated count 11→12), added missing eval-yaml config fields (`group_by`, `max_attempts`, `fail_fast`, `skill_directories`, `required_skills`, `mcp_servers`) and top-level fields (`inputs`, `tasks_from`, `hooks`, `baseline`), added Hooks/Template Variables/External Task Lists sections, fixed AZD extension URL from v0.8.0 to v0.9.0.
- **Key learning:** The `tool_constraint` grader is distinct from `behavior` — it validates specific tool names (expect/reject) while behavior tracks aggregate counts. When documenting graders, always cross-reference `internal/models/outcome.go` for the canonical `GraderKind` constant list to catch missing types. The `HooksConfig` struct in `internal/hooks/hooks.go` defines four lifecycle points: `before_run`, `after_run`, `before_task`, `after_task`.
